...
void create_directories(){
    ...
    mkdir(TEMP_FOLDER_INDIRECT_IDX_PHASE_WORDS,0777);
    mkdir(OUTPUT_FOLDER,0777);
}

void notify_wokers_of_phase(int phase){
    MPI_Status status;
    for (int worker_idx = 0; worker_idx < WORKER_SIZE; ++worker_idx){
        MPI_Send(nullptr, 0, MPI_BYTE, worker_idx, phase, MPI_COMM_WORLD);
    }
    for (int worker_idx = 0; worker_idx < WORKER_SIZE; ++worker_idx){
        MPI_Recv(nullptr, 0, MPI_BYTE, worker_idx, FINISHED_PREPARING, MPI_COMM_WORLD, &status);
    }
}
...
void master_index_phase(std::vector<std::string>* filenames_ptr, int phase){
    ...
    printf("[ MASTER ] - Etapa %s. Astept sa termine toti wrokerii\n", get_tag_name(phase));
    ...
    printf("[ MASTER ] - Etapa %s. Au terminat toti worketii\n", get_tag_name(phase));
}
